...
构建面向绿色能量与无线射频混合供能网络的仿真实验管理与异步任务调度平台。平台提供场景配置、实验提交、状态跟踪、算法执行、结果查询和失败恢复能力，并通过完整测试、运行文档和验收证据形成可用于秋招的软件开发项目。
...
第四阶段：工程化与求职材料
  Docker Compose + README + 架构图 + 演示证据 + 简历与面试材料
```
...
| 10. 工程化与求职材料 | 已完成 | README、架构图、演示材料、简历和面试材料 | 新环境可按文档启动；关键接口有测试和验收数据；简历表述可被代码与记录支撑 |

## 4. 当前里程碑

当前里程碑：**阶段10工程化与求职材料已经完成，项目当前版本正式收尾**。
...
- [x] 重写项目定位、完成状态、能力边界与技术栈；
- [x] 增加总体架构、可靠异步任务时序和GRPO跨语言推理三张Mermaid图；
- [x] 区分Java Mock、Python GRPO和MySQL扫描回退模式；
- [x] 补充本地基础设施、两种Worker启动方式、核心API与测试命令；
- [x] 记录97项Java测试、10项Python测试和真实CUDA复现数据；
- [x] 同步更新架构文档和GRPO结果API示例，删除尚未实现的产物下载接口描述。

完成记录：根README已经从早期骨架说明升级为当前可交付项目首页；所有能力、命令、测试数字和限制均来自已经实现并验收的代码，不包含在线训练、前端、对象存储或Exactly Once等未实现能力。

### 10.2：项目演示与截图证据（已完成）

- [x] 设计“无断点预演 + 关键状态取证”两遍演示路径；
- [x] 固化登录、场景创建、GRPO任务提交和结果查询请求；
- [x] 提供`experiment_task/outbox_event/task_execution/simulation_result`四表观察SQL；
- [x] 明确RabbitMQ Ready、Unacked和最终ACK的截图时机；
- [x] 创建`docs/images/`截图命名、质量与隐私规范；
- [x] 提供Java 97项和Python 10项测试截图标准；
- [x] 按手册完成完整真实运行截图；
- [x] 逐张核对状态、业务指标、图片尺寸和文件大小；
- [x] 在README中嵌入4张安全的核心运行证据；
- [x] 完成截图隐私复核：个人路径已遮盖或移除；
- [x] 确认截图中的Worker Token仅为公开的本地演示值，生产环境必须替换；
- [x] 完成截图链接和Git差异校验，正式关闭10.2。

操作手册见`docs/15-project-demo-guide.md`，截图规范见`docs/images/README.md`。真实运行截图已经完成采集和逐张核对，4张核心证据已嵌入README；个人路径已经清理，本地演示Token的公开边界已经记录，阶段10.2正式完成。

### 10.3：项目讲解材料（已完成）

- [x] 编写30秒项目快速概括；
- [x] 编写1分钟标准项目介绍；
- [x] 编写3分钟“背景—架构—难点—结果—边界”完整讲解；
- [x] 补充软件开发岗和央国企科技岗的不同讲解侧重点；
- [x] 补充常见打断问题、准确表达边界和练习验收方式。

完成记录：新增`docs/16-project-presentation-script.md`。讲解材料只使用已经实现和验收的能力，明确区分预训练推理与在线训练、至少一次投递与Exactly Once，并避免在没有压测数据时声称高并发。

### 10.4：高频面试问答（已完成）

- [x] 按结论、项目实现和边界整理44个高频问题；
- [x] 覆盖架构、事务、Outbox、RabbitMQ、幂等、状态机、Redis和异步线程；
- [x] 覆盖Java/Python职责、Worker认证、GRPO复现与测试策略；
- [x] 建立高频问题到真实源码入口的映射；
- [x] 提供三轮递进式模拟面试练习和验收标准。

完成记录：新增`docs/17-interview-qa.md`。该文档从阶段性学习日志中提炼面试高频问题，统一使用当前代码与阶段9验收后的准确口径，不替代完整的`docs/03-learning-log.md`。

### 10.5：双版本简历项目经历（已完成）

- [x] 编写软件开发岗5条标准版和3条精简版；
- [x] 编写央国企科技岗5条标准版和3条精简版；
- [x] 对比两版的岗位目标、技术表述和面试侧重点；
- [x] 补充Java、信息化、平台研发和AI应用JD的最小调整方法；
- [x] 明确可写数字、项目真实性边界和仓库公开条件。

完成记录：新增`docs/18-resume-project-entry.md`。两版项目经历基于同一份已验收事实，软件开发版侧重Java后端可靠性，央国企科技岗版侧重科研工程化、系统闭环和可追溯性；均提供一页简历可用的精简版本。
