...
仿真任务执行时间相对较长，`prefetch=1`让每个消费者同一时刻最多持有一条未ACK消息，避免一个消费者预先占住大量任务而其他消费者空闲，也形成自然背压。它不是所有系统的最优值：短小消息为了提高吞吐量通常会提高prefetch；本项目未进行容量压测，因此当前固定为1，只说明其公平分配和故障恢复作用，不声称它是性能最优值。
...
## 2026-07-19：Redis任务详情缓存与用户提交限流

### 本步完成内容

- 使用`redis:7.4.2-alpine`启动本地Redis，宿主机端口为`16379`；
- 关闭RDB和AOF，不为可重建缓存创建持久化卷；
- 接入Spring Data Redis和Lettuce，配置连接与命令超时；
- 使用`TaskDetailCache`显式实现Cache Aside，缓存键为`wireless-sim:task:detail:{userId}:{taskId}`；
- 任务详情TTL为5秒，列表查询不缓存；
- 任务状态变化后通过事务`afterCommit`回调删除缓存；
- Redis连接失败时缓存回源MySQL，并短暂退避5秒，避免Worker频繁等待连接超时；
- 使用Lua原子执行`INCR + PEXPIRE`，单用户60秒最多创建5个新任务；
- 幂等重放不消耗新任务额度，超限返回429和`TASK_SUBMISSION_RATE_LIMITED`；
- Redis不可用时限流Fail Open，不阻断任务核心业务；
- 真实Redis/MySQL测试验证连接、TTL、缓存失效、用户隔离、幂等、并发阈值和故障降级。
- 全量93项测试通过，0失败、0错误、0跳过，阶段8.7正式关闭。

### 面试可能追问

- 为什么使用Cache Aside，而不是直接给`TaskService.get`加`@Cacheable`？
- 为什么任务详情缓存键同时包含`userId`和`taskId`？
- 为什么数据库更新后删除缓存，而不是直接更新缓存？
- 为什么必须在MySQL事务提交成功后删除Redis缓存？
- 5秒TTL是否能完全解决缓存一致性问题？
- Redis不可用时为什么选择Fail Open？
- 为什么限流中的`INCR`和`PEXPIRE`要放进Lua脚本？
- 固定窗口限流有什么边界突发问题？
- 为什么幂等重放不占用新的提交额度？
- 为什么本阶段不使用Redis分布式锁？
...
任务详情读取需要把“命中、未命中、JSON损坏、连接异常、用户隔离和降级”全部变成可观察、可测试的业务边界。显式适配器让Redis异常不会从注解代理直接传播，也方便理解缓存不是事实来源。Spring Cache注解适合常规缓存，但本项目第一版更重视故障语义和学习可解释性。

#### 为什么事务提交后才删除缓存？

如果在MySQL提交前删除缓存，另一个请求可能立刻回源并把事务提交前的旧数据重新写入缓存；如果事务最终回滚，这次删除也没有必要。项目通过`TransactionSynchronization.afterCommit`在数据库确认提交后删除，Redis删除失败则依靠5秒TTL最终收敛。

#### 为什么限流使用Lua？

`INCR`和首次设置TTL如果是两个独立客户端命令，应用可能在两者之间宕机，留下永不过期的计数器。Lua在Redis服务器端把递增、判断首次访问和`PEXPIRE`作为一个原子操作执行，并发请求不会越过5次阈值。

#### 为什么Redis不可用时Fail Open？

缓存和限流都是保护与优化能力，不是任务事实。Fail Closed会让Redis故障直接阻断所有科研任务提交；Fail Open允许读取回源MySQL并暂时放行提交，核心正确性继续由数据库幂等键、唯一约束、任务状态机和RabbitMQ消费幂等保证。代价是Redis故障期间暂时失去流量保护，因此生产环境还需要监控和告警。

#### 为什么不使用Redis分布式锁？

任务执行互斥已经由MySQL带状态和轮次条件的原子更新、执行记录唯一约束完成。再增加Redis锁会引入锁超时、续期、误释放和双重事实来源问题，却不能提高当前正确性。Redis只承担可丢失的缓存和短期计数职责。

## 2026-07-20：阶段8综合故障测试与最终验收

### 本步完成内容

- 新增脏缓存测试：Redis被写入非法JSON时，删除损坏键、回源MySQL并重建正确缓存；
- 扩展事务测试：Outbox插入失败使任务重试事务回滚时，`afterCommit`缓存删除回调不会执行；
- 新增真实端到端测试：任务提交后经过Outbox、RabbitMQ、消费者和Java Worker，最终四张业务表状态一致；
- 实际重启Redis，确认无持久化缓存键消失且连接恢复；
- 实际重启RabbitMQ，确认3个持久化队列和拓扑恢复；
- 全量95项测试通过，并确认MySQL、Redis和RabbitMQ无测试残留；
- 阶段8达到“At-Least-Once + 业务幂等 + 异常可恢复 + 缓存可降级”的验收口径。

### 面试可能追问

- 如何测试ACK前应用宕机，而不必真的在每次测试中杀死JVM？
- 为什么Redis缓存内容损坏时不能直接向上抛异常？
- 为什么事务回滚时不能删除缓存？
- RabbitMQ重启后队列还在，是否就证明消息一定不会丢？
- 如何证明Outbox、任务、执行记录和结果之间最终一致？
- 为什么端到端测试仍不能证明系统是Exactly Once？
- Redis不持久化是不是不可靠设计？
- 阶段8验收后还存在哪些生产边界？

#### 如何测试ACK前应用宕机窗口？

监听器只有在Worker把任务状态、执行记录和结果可靠落库后才ACK。若应用在ACK前退出，RabbitMQ会重新投递未确认消息。自动化测试通过再次发送同一业务轮次消息模拟等价的重复投递结果，并验证任务状态条件、`attemptNo`和`task_execution(task_id, attempt_no)`唯一约束只允许一条执行记录和一份结果。真实杀进程可用于演示，但不适合作为每次构建都执行的确定性测试。

#### 为什么缓存内容损坏时要回源MySQL？

Redis不是事实来源，损坏的JSON可能来自格式升级、人工误操作或旧版本残留。把反序列化异常直接返回给用户会让一个可丢弃的优化层破坏核心查询。正确做法是记录告警、删除脏键、读取MySQL真值并重新缓存，从而让故障自动收敛。

#### 为什么事务回滚时不能删除缓存？

回滚表示MySQL事实没有变化，原缓存仍与数据库一致。如果写操作尚未提交就删除缓存，会产生不必要的回源；更重要的是，其他线程可能在事务结束前读到旧数据并重新写缓存。`afterCommit`保证只有数据库新状态真实生效后才执行失效。

#### RabbitMQ重启后队列仍存在，是否等于消息绝不丢失？

不等于。队列和交换机必须声明为durable，消息还必须是persistent，Broker也需要持久化存储；生产环境还要考虑磁盘故障、集群副本和确认策略。本项目重启演练证明本地单节点的持久化拓扑可恢复，Publisher Confirm证明Broker接管消息，但不把它夸大为跨节点零丢失保证。

#### 如何证明四张业务表最终一致？

端到端测试从真实`TaskService.submit`开始，先断言任务与Outbox均为`PENDING`，再手动触发发布并等待异步消费完成。最后同时核对Outbox为`PUBLISHED`、任务为`SUCCEEDED`且进度100、执行记录恰好一条且心跳与起止时间完整、结果恰好一条且核心指标非空，同时验证旧缓存失效。它验证的是一个完整业务链路的可观察不变量，而不是只测单个方法返回值。

#### 为什么仍然不是Exactly Once？

RabbitMQ发布、Outbox状态更新、消费和ACK跨越不同事务边界。发布成功但Outbox未更新、或业务落库成功但ACK前退出，都可能产生重复消息。系统接受至少一次投递，再用数据库业务幂等保证重复消息不产生重复业务结果。因此准确表述是“At-Least-Once delivery + Effectively-Once business effect”，不能宣称消息只传输一次。

#### Redis不持久化是不是不可靠？

是否持久化取决于数据职责。本项目Redis只保存5秒任务详情缓存和60秒限流计数，两者都允许丢失；MySQL保存全部业务事实。关闭RDB/AOF可减少本地磁盘占用和恢复复杂度，重启后缓存通过MySQL自然重建。若未来Redis保存不可重建会话或队列数据，就必须重新评估持久化和高可用方案。

#### 阶段8完成后还有哪些边界？

当前仍是本地单节点MySQL、RabbitMQ和Redis，死信主要依靠人工处理，也没有接入指标监控、压测或跨机器部署。阶段9随后接入了Python/PyTorch预训练GRPO推理，阶段10完成README、架构图、演示证据、简历和面试材料；监控、压测和跨机器部署不纳入当前版本范围。

## 2026-07-20：GRPO预训练模型独立推理程序

### 本步完成内容

- 把现有GRPO科研代码整理为独立Python模块，保留训练代码但平台第一版只调用评估；
- 移除评估脚本中的本机C盘绝对路径，支持CLI和环境变量指定权重、输出目录；
- 修复从仓库根目录启动时找不到`algo`模块的问题；
- 固定第一版只接受3设备RSMA场景，并校验15维观测和13维动作；
- 支持覆盖与Java场景对应的连续环境参数，并明确W、mW、mJ、Mb单位；
- 不改变`RsmaEnv.DEFAULTS`中的任何物理参数值；
- 使用可信本地权重、`torch.inference_mode()`和确定性策略完成推理；
- 输出标准JSON，记录权重SHA-256、场景、吞吐量统计、AoI空值和产物目录；
- 修复`reset(seed=...)`接收seed却没有真正重置随机状态的问题；
- 真实CUDA运行成功，3项自动化测试通过，相同输入吞吐量指标可复现。

### 面试可能追问

- 为什么只加载预训练权重仍然需要模型结构和环境代码？
- 为什么3设备权重不能直接评估5设备场景？
- 为什么推理使用`torch.inference_mode()`和`model.eval()`？
- 为什么只在程序启动时调用一次`np.random.seed()`还不够？
- 为什么结果中要记录权重SHA-256？
- 为什么AoI必须返回空，而不能生成一个占位数字？
- 为什么Python不直接更新MySQL？

#### 为什么权重不能脱离模型结构和环境运行？

权重只保存网络参数。评估还需要一致的Actor结构、输入输出维度、动作缩放、状态含义和环境转移规则。结构或环境不一致时，即使张量维度勉强匹配，结果也可能失去原训练语义。

#### 为什么当前模型只支持3个设备？

当前环境观测维度为`5 × 设备数`，动作维度为`1 + 4 × 设备数`。权重第一层输入为15、输出为13，反推出训练设备数为3。改成5设备后维度变为25和21，原网络参数无法严格加载，因此入口在推理前明确拒绝。

#### `inference_mode`和`eval`分别解决什么问题？

`model.eval()`切换Dropout、BatchNorm等模块的推理行为；`torch.inference_mode()`关闭梯度记录及相关版本跟踪，减少推理内存和开销。两者职责不同，评估时通常同时使用。

#### 为什么程序开头设置一次seed仍不能保证重复调用可复现？

同一进程第一次评估会持续消耗NumPy随机序列，第二次评估若环境reset不重新应用回合seed，就会从剩余状态继续采样。修复后每回合用`baseSeed + episodeIndex`重置Python、NumPy和PyTorch随机状态，相同任务的业务指标才能稳定复现。

#### 为什么记录权重SHA-256？

文件名相同不代表内容相同。SHA-256把评估结果绑定到准确的权重字节，出现结果差异时可以区分是场景、代码还是模型文件发生变化，也避免误加载被替换的本地权重。

#### 为什么AoI保持为空？

当前环境奖励和评估脚本只实现吞吐量，未实现AoI状态和计算公式。填0或随机值会把“没有计算”误表达成“真实AoI为0”。使用JSON `null`和数据库NULL可以准确表达该指标不适用。

## 2026-07-20：Java/Python预训练GRPO异步闭环

### 本步完成内容

- 设计领取、成功、失败三个内部Worker接口，删去不适合秒级推理的进度与运行中取消接口；
- 使用独立`X-Worker-Token`保护服务间API，与用户JWT职责分离；
- Java领取时校验GRPO/吞吐量/RSMA/3设备边界，并返回单位明确的场景快照；
- Python使用RabbitMQ手动ACK消费任务，调用现有PyTorch推理函数后回调Java；
- Java通过单个事务完成任务、执行记录和结果表的成功闭环，AoI明确保存NULL；
...
- 为什么Python不直接读取或写入MySQL？
- 为什么秒级推理不做实时进度与运行中取消？
- 为什么领取接口需要返回`RESUMABLE`？
- Worker Token与用户JWT有什么区别？
- Python什么时候才能ACK RabbitMQ消息？
- Python推理完成但成功回调超时会发生什么？
- 为什么场景字段名称中要包含单位？
- 为什么模型权重SHA-256要与结果一起保存？
- `worker-mode`为什么是必要配置？
- 这套链路能否宣称Exactly Once？

#### 为什么Python不直接访问MySQL？

任务状态机、执行记录、结果唯一约束和缓存失效已经由Java统一管理。如果Python也直接改表，会形成两个业务写入口，难以保证事务边界和状态转换一致。Python只负责计算，Java通过内部API在单个事务中落库，使数据库规则仍集中在一个服务中。

#### 为什么不做逐回合进度和运行中取消？

当前评估通常只需几秒。频繁回调会增加网络请求、数据库事务和部分失败状态，却很难给用户带来实际价值。第一版只记录领取和终态；进程崩溃由已有心跳超时恢复兜底。准确口径是“不支持Python推理中途取消”，而不是假装具备实时控制能力。

#### 为什么需要RESUMABLE？

Python可能已经领取并算完，但成功回调时Java短暂不可访问，RabbitMQ随后会重投同一消息。任务此时是RUNNING，若简单返回“已处理”并ACK，结果就永远无法落库。`RESUMABLE`允许同一轮次重新推理并再次回调；数据库条件更新和结果唯一约束保证最终只产生一个业务结果。

#### Worker Token与JWT的区别是什么？

JWT表示具体登录用户，包含用户身份和角色，用于外部业务API；Worker Token表示受信任的内部计算进程，不对应某个用户。分离后Python无需保存用户令牌，也不能借服务凭证调用普通用户接口。当前静态Token适合本地演示，生产可升级为mTLS或短期服务身份。

#### Python什么时候ACK？

只有Java已可靠保存成功/失败终态，或明确返回旧消息、重复消息已经被幂等吸收时才能ACK。若Java网络错误，Python先把原消息可靠发布到重试队列并获得Publisher Confirm，再ACK原消息；否则应保留原消息，避免静默丢失。

#### 回调超时为什么可能重新计算？

HTTP超时无法判断Java是否已收到并提交。系统选择At-Least-Once：允许重投和重复计算，再依靠任务状态条件、执行轮次唯一约束和结果唯一约束吸收重复业务效果。对当前秒级推理而言，少量重复计算比丢失结果更可接受。

#### 为什么字段名直接携带单位？

`P_wpt=4`如果没有单位，Python可能按mW解释而Java按W保存，数值合法但科研语义完全错误。`wptTransmitPowerWatt`、`batteryCapacityMilliJoule`等名称让契约自描述，避免在两个语言中散落隐式换算规则。

#### worker-mode解决了什么问题？

RabbitMQ同一队列上的消费者是竞争关系。如果Java Mock监听器和Python Worker同时启动，一部分任务会被Java生成合成结果，另一部分才进入真实模型，结果不可预测。`java-mock/python`互斥配置让部署者明确选择唯一执行实现。
