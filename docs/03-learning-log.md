...
消息只携带`eventId`、`taskId`、`eventType`、`schemaVersion`和`occurredAt`，Worker再根据`taskId`从MySQL读取任务快照。这样可以减少消息体积、避免消息中的参数与数据库事实不一致，也降低以后字段演进和敏感数据泄露风险。代价是消费者需要查询一次数据库，但本项目的任务执行本来就依赖数据库状态抢占，因此这个取舍更合适。
